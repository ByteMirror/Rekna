...
describe("sheet autocomplete", () => {
  ...
  test("only shows importable cross-sheet symbols while editing an Import line", () => {
    const importState = EditorState.create({ doc: "Import sub" });
    const importContext = new CompletionContext(
      importState,
      "Import sub".length,
      false
    );
    const normalState = EditorState.create({ doc: "sub" });
    const normalContext = new CompletionContext(
      normalState,
      "sub".length,
      false
    );

    const importResult = sheetCompletionSource(importContext, {
      externalSymbols: [],
      importableSymbols: [
        {
          kind: "object",
    ...
    const normalResult = sheetCompletionSource(normalContext, {
      externalSymbols: [],
      importableSymbols: [
        {
          kind: "object",
    ...
    expect(
      importResult?.options.some((option) => option.label === "subscriptions")
    ).toBe(true);
    expect(
      normalResult?.options.some((option) => option.label === "subscriptions")
    ).toBe(false);
  });
  ...
  test("includes shared alias completions for operators and currencies", () => {
    const state = EditorState.create({ doc: "" });
    const context = new CompletionContext(state, 0, true);
    const result = sheetCompletionSource(context);

    expect(
      result?.options.find((option) => option.label === "plus")?.detail
    ).toBe("Operator");
    expect(
      result?.options.find((option) => option.label === "euro")?.detail
    ).toBe("Currency");
    expect(result?.options.find((option) => option.label === "€")?.detail).toBe(
      "Currency"
    );
  });
  ...
  test("offers workspace tag completions with a dedicated tag type", () => {
