...
export function tokenizeSheetLine(
  ...
  const directiveMatch = line
    .slice(contentStart)
    .match(
      /^(\s*)(Import|Export)(\s+)([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*(?:\s*,\s*[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*)*)$/i
    );
  if (directiveMatch?.[2] && directiveMatch[4]) {
    ...
    const symbolsStart = keywordEnd + (directiveMatch[3]?.length ?? 0);
    ...
    addDirectiveListTokens(tokens, directiveMatch[4], symbolsStart, variables);

    return tokens.sort((left, right) => left.from - right.from);
  }

  addReversePercentagePhraseTokens(tokens, line);
  addMultiWordUnitAliasTokens(tokens, line);
...
function addDirectiveListTokens(
  tokens: SheetSyntaxToken[],
  text: string,
  from: number,
  variables: ReadonlySet<string>
  ...
  for (const symbol of text.split(/\s*,\s*/)) {
    ...
    const symbolStart = text.indexOf(trimmedSymbol, offset);

    if (symbolStart < 0) {
      continue;
    }

    addDirectiveSymbolToken(
      tokens,
      trimmedSymbol,
      from + symbolStart,
      variables
    );
    offset = symbolStart + trimmedSymbol.length;
  }
}

function addDirectiveSymbolToken(
  tokens: SheetSyntaxToken[],
  text: string,
  from: number,
  variables: ReadonlySet<string>
...
function addReversePercentagePhraseTokens(
  tokens: SheetSyntaxToken[],
  line: string
) {
  ...
  const keywordGroups = match.indices.slice(1, 4);
...
function addMultiWordUnitAliasTokens(tokens: SheetSyntaxToken[], line: string) {
  for (const phrase of MULTI_WORD_UNIT_ALIAS_PHRASES) {
