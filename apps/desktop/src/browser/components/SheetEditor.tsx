...
const MAX_RESULTS_COLUMN_WIDTH_CH = 24;
...
export function SheetEditor({
  ...
  const latestImportableSymbolsRef = useRef(importableSymbols);
  ...
  latestImportableSymbolsRef.current = importableSymbols;
  latestWorkspaceTagsRef.current = workspaceTags;

  const toastPortalTarget = resolveToastPortalTarget(rootRef.current);
          ...
          override: [
            (context) =>
              sheetCompletionSource(context, {
                ...
                importableSymbols: latestImportableSymbolsRef.current,
...
function handleObjectNewline(view: EditorView) {
  ...
  const changes = view.state.changeByRange((range) => {
    const indentLevel = getObjectIndentLevelAtPosition(
      documentText,
      range.from
    );
    ...
    didIndent = true;
    const insert = view.state.toText(
      `${view.state.lineBreak}${"\t".repeat(indentLevel)}`
    );
...
function getObjectIndentLevelAtPosition(text: string, position: number) {
  let indentLevel = 0;
  let inComment = false;
...
function resolveToastPortalTarget(rootElement: HTMLElement | null) {
  if (!rootElement) {
...
function resultClassName(line: EvaluatedLine) {
  const baseClassName =
    "flex h-full items-center justify-end whitespace-nowrap tabular-nums";
...
function getResultsColumnWidthCh(lines: EvaluatedLine[]) {
  const contentWidthCh = Math.max(
    12,
    ...lines.map((line) => measureResultDisplayWidth(line))
  );

  return Math.min(MAX_RESULTS_COLUMN_WIDTH_CH, Math.ceil(contentWidthCh + 4));
}

function formatResultsColumnWidth(widthCh: number) {
  return `max(11rem, ${widthCh}ch)`;
}

function measureResultDisplayWidth(line: EvaluatedLine) {
  if (getDirectiveStatus(line)) {
    return 2.5;
  }

  return measureDisplayWidth(line.displayValue);
}

function measureDisplayWidth(displayValue: string | null) {
  if (!displayValue) {
    return 0;
  }
