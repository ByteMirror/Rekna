...
const { SheetEditor, buildResultLineSlots } = await import("./SheetEditor");
...
describe("SheetEditor", () => {
  ...
  test("copies a clicked result and shows a confirmation toast", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        ...
        expect(status.textContent).toBe("Copied 20");
        expect(sheetScroller.contains(status)).toBe(false);
        expect(status.parentElement?.className).toContain("fixed");
      });
    } finally {
      cleanup();
      window.document.body.innerHTML = "";
  ...
  test("indents a new line inside an object block when Enter is pressed", async () => {
    const value = ["subscriptions = {", "}"].join("\n");
  ...
  test("routes Mod-f to sheet search instead of opening the editor find panel", async () => {
    const handleOpenSheetSearch = mock((_event: Event) => {});
    ...
    try {
      ...
      await waitFor(() => {
        expect(handleOpenSheetSearch).toHaveBeenCalledTimes(1);
      });
  ...
  test("comments selected lines with // when Mod-/ is pressed", async () => {
  ...
  test("comments selected lines with // when slash requires Shift on the keyboard layout", async () => {
    const value = ["first = 1", "second = 2", "third = 3"].join("\n");
    const onChange = mock((_value: string) => {});
  ...
  test("clears stale external autocomplete entries when switching to a blank document", async () => {
    try {
      const firstValue = "sub";
  ...
  test("widens the results column for long calculated output", async () => {
    try {
      const evaluation = await evaluateSheet(
      ...
      expect(layout.getAttribute("style")).toContain(
        "grid-template-columns: minmax(0, 1fr) max(11rem,"
      );
    } finally {
      cleanup();
      window.document.body.innerHTML = "";
  ...
  test("caps the results column width for unexpectedly long output", () => {
    try {
      const lines: SheetEditorLine[] = [
        {
      ...
      expect(getByTestId("sheet-layout").getAttribute("style")).toContain(
        "grid-template-columns: minmax(0, 1fr) max(11rem, 24ch);"
      );
    } finally {
      cleanup();
      window.document.body.innerHTML = "";
  ...
  test("keeps the widened results slot stable when later output becomes shorter", async () => {
    try {
      ...
      await waitFor(() => {
        ...
        expect(nextStyle).not.toContain("max(11rem, 16ch)");
